...
- Текущее состояние: секции «Clusters» / «Standalone hosts» + пользовательские группы поверх (B16)
- Вопросы: где место хранилищ (per-node vs отдельная секция vs в группе), вложенность
  кластер → ноды → ВМ/CTL, группы кластера/хоста, пулы как подуровень?
- Согласовать с VISION.md (Site/Datacenter) и сводками (SUMMARY/agreggated cards)

### B19. Примечания к элементам дерева
...
- Где хранить: config.sqlite рядом с полем `group` (B16); для ВМ — ключ по host+vmid
- Как редактировать: контекстное меню дерева «Edit note...» / инлайн-переименование?
- Как отображать: вторая строка / суффикс после имени, приглушённым стилем
- Не путать с тегами ВМ из PVE (это локальная метка, PVE не трогаем)
- Связано с B18 (структура дерева) и глобальным поиском B14 (индексировать заметку)
