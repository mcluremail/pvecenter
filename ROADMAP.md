...
> **Порядок согласован: M0 → M1 → M2 → M3 → M4 → далее по списку
> (порядок M5–M10 обсудим; M10 «UI-паритет» — в конец, приоритеты определим позже).**
...
#### M3. Cross-cluster move ⏳ — киллер-фича (см. B21)
- Мастер «Move VM/CT to cluster…»: основной путь — `remote_migrate`
  (API есть для qemu+lxc, помечен EXPERIMENTAL, PVE 8+): `target-endpoint`
  собирается из нашего конфига подключения целевого кластера,
  автоподбор `target-storage`/`target-bridge` по хранилищам, видимым
  обоим сторонам, + ручная правка; `target-vmid`, `online` (live),
  `delete` исходника — опция с явным предупреждением.
- Fallback — offline через PBS: restore снапшота в целевом кластере
  (фундамент B17/v2.12 — restore PBS-бэкапа в новую ВМ реализован).
- Формат endpoint (apitoken/host/fingerprint) сверить по исходникам
  pve-manager при реализации; прогресс по task-UPID обоих кластеров.
- Единственный в мире GUI-способ миграции между независимыми PVE без
  установки сервера (у офиц. UI кнопки нет, у PDM — server+web).

#### M4. Maintenance mode ⏳ (см. B22)
...
#### M6. Дерево 3.x ⏳ (закрывает follow-up'ы B18/B20)
...
#### M10. UI-паритет с офиц. PVE ⏳ — в конец списка, приоритеты определим (см. B23)
...
- B10 Replication: zfs replication между нодами
  (`GET/POST/DELETE /nodes/{node}/replication`) — кандидат в M9+.
...
badge в M10 достаточно); **DRS/load balancing** (XO/ProxCenter-стиль —
