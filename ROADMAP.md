...
- **M1.2. Темы (след.)** — по решению владельца: мимикрия под KDE —
  Breeze, Breeze Dark, Oxygen + свой «Нейтральный графит»; палитры снять
  из исходников KDE breeze (точные значения, не по памяти); акцент
  Breeze `#3daee9`; Breeze/Breeze Dark — собственные наборы иконок
  24px (элементы интерфейса станут чуть крупнее — плотность через
  `extra_qss()`); «Системная» (colorScheme, слушатель стиля) — вместе
  с появлением dark.
