...
- **M1.3. Публичный контракт** — `docs/THEMES.md` (v1: 36 токенов +
  точки расширения), preview в UI, импорт сторонних .py-тем; JSON —
  после 3.0. Строка меню (пункты Theme/Settings) — отдельный пункт
  ROADMAP после M1, в веху не входит.

#### M2. Командная палитра (Ctrl+K) ⏳
- Единый реестр действий на **QAction + метаданных** (действие, контекст,
  shortcut — декларативно, без хрупкого списка сигналов mainwindow);
  палитра = fuzzy-поиск по реестру; контекстные фильтры («действия над
  выделенной ВМ», «перейти к ноде»).
...
- **M4.0. Тестовый харнесс до фич**: fake PVE/PBS API + record/replay
  fixture'ы — Fleet Health фан-аутится по всем кластерам сразу, живыми
  кластерами не тестируем (до этого 806 тестов не покрывают API-регрессии).
...
- Storage runway: тренды rrddata → «хранилище кончится через ~N дней»
  (capacity forecasting из идей влилась сюда); прогноз с доверительным
  интервалом — rrddata дырявый, наивная линейность врёт.
- Snapshot sprawl: забытые старые снапшоты по всему парку (per-VM запрос,
  скан по кнопке + кэш, лимит параллелизма).
- Partial failure: кластер offline → плашка «данные от HH:MM» (последнее
  известное), отчёт не становится пустым.
