...
При ошибках DNS приложение корректно помечает хост как `error` и **не блокирует загрузку остальных** — дерево обновляется по мере ответов. Хост не пропадает из дерева (остаётся на месте со старым статусом), а в панели деталей показывается сообщение с диагнозом: "Не удаётся разрешить DNS-имя хоста".
...
### 8. Диагностика ошибок в интерфейсе

При клике на хост с ошибкой приложение показывает в панели деталей сообщение с автоматическим определением причины:

| Текст ошибки | Что пишет в интерфейсе |
|---|---|
| `resolve`, `dns`, `name or service not known` | Не удаётся разрешить DNS-имя хоста |
| `connection refused`, `connection reset` | PVE API недоступен (соединение отклонено) |
| `timeout` | Хост не отвечает (таймаут соединения) |
| `unauthorized`, `401`, `permission denied` | Ошибка авторизации API-токена |
| `403` | Недостаточно прав API-токена |
| Всё остальное | Выводится сырой текст ошибки |

При этом не показываются графики, таблицы и вкладки — только сообщение. После восстановления связи на следующем рефреше данные подтягиваются автоматически.
...
Для кластерных нод `_display_name` формируется как `short@cluster` (`pve02@production`), без FQDN — все запросы идут через мастер-ноду кластера.
